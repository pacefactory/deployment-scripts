...
`PostgreSQL`, `SQL` (client database via relational_dbserver: TDS for `mssql`, PostgreSQL wire for `postgres`, per `connection.json`), `SSH`, `file` (bind
...
| `relational_dbserver` | `relational_dbserver` | `pacefactory/relational-dbserver` | [`rdb`](profiles/rdb.md) | [scv2_relational_dbserver](https://github.com/pacefactory/scv2_relational_dbserver) | internal | Express HTTP API that runs SQL queries defined in JSON config files against a client's SQL Server or PostgreSQL database (TypeORM) and returns the rows |
...
| Client SQL database | `ext_client_sql` | SQL (TDS via the mssql driver, or PostgreSQL wire via pg; chosen per database by connection.json type) | out | [client-sql.md](integrations/client-sql.md) | A client's existing SQL Server or PostgreSQL database queried by relational_dbserver |
