---
title: "Reference deployment: Offline processing"
type: reference
derived_from:
  - docs/architecture/reference-deployments/offline-processing/docker-compose.built.yml
  - docs/architecture/reference-deployments/offline-processing/.env
  - docs/architecture/reference-deployments/offline-processing/.settings
...
The default build with the `offline` profile: dbserver autodelete disabled and audit processing in offline mode, for running recorded video through the Offline Processing tool.

Configuration-only difference from the default build; no extra services.
...
```bash
./scripts/docs/build-reference-deployment.sh offline-processing          # rewrite docker-compose.built.yml
./scripts/docs/build-reference-deployment.sh offline-processing --check  # exit 1 if the committed output is stale
```
...
```bash
docker compose --project-name deployment-scripts --env-file .env --profile base --profile mqtt-public --profile expresso-010 --profile node-red --profile offline --profile rdb --profile social --profile tools -f compose/docker-compose.base.yml -f compose/docker-compose.mqtt-public.yml -f compose/docker-compose.expresso-010.yml -f compose/docker-compose.node-red.yml -f compose/docker-compose.offline.yml -f compose/docker-compose.rdb.yml -f compose/docker-compose.social.yml -f compose/docker-compose.tools.yml config
```
...
```bash
declare -A SCV2_PROFILES=([custom]="true" [rdb]="true" [mqtt-public]="true" [expresso-010]="true" [base]="true" [social]="true" [node-red]="true" [offline]="true" [mqtts-public]="true" [tools]="true" )
...
| `offline` | prompted, default off | [offline](../../profiles/offline.md) |
...
flowchart LR
  ...
  subgraph deployment["Reference deployment: offline-processing (project deployment-scripts)"]
  ...
  class nodered,relational_dbserver,social_video_server,social_web_app optional
...
flowchart LR
  classDef optional stroke-dasharray: 5 5
  subgraph deployment["offline-processing"]
    realtime["realtime"]
    dbserver["dbserver"]
    pf_mosquitto["pf_mosquitto"]
