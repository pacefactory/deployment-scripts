...
derived_from:
  ...
  - docs/architecture/reference-deployments/https-cert-file/.env
  - docs/architecture/reference-deployments/https-cert-file/.settings
...
```bash
./scripts/docs/build-reference-deployment.sh https-cert-file          # rewrite docker-compose.built.yml
./scripts/docs/build-reference-deployment.sh https-cert-file --check  # exit 1 if the committed output is stale
```
...
MQTTS_CERT_SOURCE=../credentials/ssl
```
...
flowchart LR
  ...
  subgraph deployment["Reference deployment: https-cert-file (project deployment-scripts)"]
  ...
  ext_host_fs -->|"bind /home/pacefactory/scv2/git_clones/deployment-scripts/credentials/ssl -> /etc/nginx/ssl (ro)"| apigateway
  ...
  ext_host_fs -->|"bind /home/pacefactory/scv2/git_clones/deployment-scripts/credentials/ssl -> /etc/mosquitto-tls (ro)"| pf_mosquitto
...
Source: [`request-flows.mmd`](request-flows.mmd) (template `scripts/docs/request-flows/tls-file.mmd`). Intra-service steps are collapsed to one note; see the owning service repo for internals.
