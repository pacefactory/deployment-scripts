...
derived_from:
  ...
  - docs/architecture/reference-deployments/alerting/.env
  - docs/architecture/reference-deployments/alerting/.settings
  - docs/architecture/reference-deployments/alerting/build-command.txt
...
The default build plus the Alert Processing Engine, which force-enables `rdb` and `expresso-010`. Autozone is deliberately not included.

APE publishes its two HTTP ports on ephemeral host ports.
...
```bash
./scripts/docs/build-reference-deployment.sh alerting          # rewrite docker-compose.built.yml
./scripts/docs/build-reference-deployment.sh alerting --check  # exit 1 if the committed output is stale
```
...
```bash
docker compose --project-name deployment-scripts --env-file .env --profile ape --profile base --profile mqtt-public --profile expresso-010 --profile node-red --profile rdb --profile social --profile tools -f compose/docker-compose.ape.yml -f compose/docker-compose.base.yml -f compose/docker-compose.mqtt-public.yml -f compose/docker-compose.expresso-010.yml -f compose/docker-compose.node-red.yml -f compose/docker-compose.rdb.yml -f compose/docker-compose.social.yml -f compose/docker-compose.tools.yml config
```

## Recorded inputs

Notable overrides: none beyond the default build.
...
```bash
declare -A SCV2_PROFILES=([custom]="true" [rdb]="true" [ape]="true" [mqtt-public]="true" [expresso-010]="true" [base]="true" [social]="true" [node-red]="true" [mqtts-public]="true" [tools]="true" )
...
| Profile | Class | Page |
|---|---|---|
| `ape` | prompted, default off | [ape](../../profiles/ape.md) |
...
|---|---|---|---|---|---|---|
| `alert_processing_engine` | `alert_processing_engine` | `pacefactory/alert_processing_engine:latest` | [`ape`](../../profiles/ape.md) | external_network | ephemeral->5380 |  |
| `ape_frame_playback` | `ape_frame_playback` | `pacefactory/alert_processing_engine:latest` | [`ape`](../../profiles/ape.md) | external_network | ephemeral->5381 |  |
| `ape_timescaledb` | `ape_timescaledb` | `timescale/timescaledb:latest-pg16` | [`ape`](../../profiles/ape.md) | external_network | none |  |
...
| `celery_beat` | `celery_beat` | `pacefactory/expresso_server:latest` | [`expresso-010`](../../profiles/expresso-010.md) | expresso_network | none |  |
| `celery_worker` | `celery_worker` | `pacefactory/expresso_server:latest` | [`expresso-010`](../../profiles/expresso-010.md) | expresso_network, external_network | none |  |
...
| `expresso_server` | `expresso_server` | `pacefactory/expresso_server:latest` | [`expresso-010`](../../profiles/expresso-010.md) | expresso_network, external_network | none |  |
...
|---|---|---|---|
| ephemeral | `alert_processing_engine:5380` | tcp | host port assigned by Docker at start |
| ephemeral | `ape_frame_playback:5381` | tcp | host port assigned by Docker at start |
...
| Volume | Mounted by |
|---|---|
| `ape-data` | `alert_processing_engine`, `ape_frame_playback` |
| `ape_timescaledb-data` | `ape_timescaledb` |
...
| `expresso-data` | `celery_worker`, `expresso_server` |
| `mongodata` | `mongo` |
...
flowchart LR
  ...
  subgraph deployment["Reference deployment: alerting (project deployment-scripts)"]
  ...
  port_eph_alert_processing_engine_5380[/"host :ephemeral"/] -->|"tcp -> alert_processing_engine:5380"| alert_processing_engine
  port_eph_ape_frame_playback_5381[/"host :ephemeral"/] -->|"tcp -> ape_frame_playback:5381"| ape_frame_playback
  ...
  vol_ape_data[("volume: ape-data")]
  alert_processing_engine -->|"/home/scv2/ape (rw)"| vol_ape_data
  ape_frame_playback -->|"/home/scv2/ape (rw)"| vol_ape_data
  vol_ape_timescaledb_data[("volume: ape_timescaledb-data")]
  ape_timescaledb -->|"/var/lib/postgresql/data (rw)"| vol_ape_timescaledb_data
  ...
  class alert_processing_engine,ape_frame_playback,ape_timescaledb,nodered,relational_dbserver,social_video_server,social_web_app optional
...
flowchart LR
  ...
  class social_web_app,ape_frame_playback,ape_timescaledb,alert_processing_engine,nodered,social_video_server,relational_dbserver optional
```

### Request flows

Source: [`request-flows.mmd`](request-flows.mmd) (template `scripts/docs/request-flows/alerting.mmd`). Intra-service steps are collapsed to one note; see the owning service repo for internals.
