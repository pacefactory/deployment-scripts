---
title: Reference deployment: Default build
type: reference
derived_from:
  - docs/architecture/reference-deployments/default-build/docker-compose.built.yml
  - docs/architecture/reference-deployments/default-build/.env
  - docs/architecture/reference-deployments/default-build/.settings
...
What `./build.sh -q` produces on a fresh checkout with no `.settings` and no `.env`: the forced profiles plus every default-on profile.

Baseline every other reference deployment is diffed against.
...
```bash
./scripts/docs/build-reference-deployment.sh default-build          # rewrite docker-compose.built.yml
./scripts/docs/build-reference-deployment.sh default-build --check  # exit 1 if the committed output is stale
```
...
```bash
docker compose --project-name deployment-scripts --env-file .env --profile base --profile mqtt-public --profile expresso-010 --profile node-red --profile rdb --profile social --profile tools -f compose/docker-compose.base.yml -f compose/docker-compose.mqtt-public.yml -f compose/docker-compose.expresso-010.yml -f compose/docker-compose.node-red.yml -f compose/docker-compose.rdb.yml -f compose/docker-compose.social.yml -f compose/docker-compose.tools.yml config
```

## Recorded inputs

Notable overrides: mongo sizing pinned to the 16 GB host tier.
...
```bash
declare -A SCV2_PROFILES=([custom]="true" [rdb]="true" [mqtt-public]="true" [expresso-010]="true" [base]="true" [social]="true" [node-red]="true" [mqtts-public]="true" [tools]="true" )
...
flowchart LR
  ...
  subgraph deployment["Reference deployment: default-build (project deployment-scripts)"]
...
flowchart LR
  classDef optional stroke-dasharray: 5 5
  subgraph deployment["default-build"]
    realtime["realtime"]
    dbserver["dbserver"]
    pf_mosquitto["pf_mosquitto"]
